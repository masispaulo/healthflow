...
import PatientsModal from './PatientsModal'; // <--- Importamos o Modal que criamos
...
const ProcedureInput: React.FC<ProcedureInputProps> = ({ shiftId, onUpdate }) => {
  ...
  // Controle do Modal de Pacientes
  ...
  // Callback quando seleciona um paciente na lista
  const handleSelectPatient = (patient: any) => {
      setName(`${patient.name} (Pront: ${patient.recordNumber})`);
      setType('WORK'); // Assume que se selecionou paciente, é trabalho
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className={`grid grid-cols-1 md:grid-cols-4 gap-4 items-end p-4 rounded-lg border transition-colors ${type === 'WORK' ? 'bg-slate-750 border-slate-700' : type === 'SLEEP' ? 'bg-indigo-900/10 border-indigo-500/30' : 'bg-yellow-900/10 border-yellow-500/30'}`}>
        ...
        <div className="md:col-span-1 space-y-1 relative">
          <label className="text-xs text-slate-400 uppercase font-bold flex justify-between">
              ...
              {type === 'WORK' && (
                  <span className="text-[10px] text-indigo-400 cursor-pointer hover:underline" onClick={() => setIsPatientModalOpen(true)}>
                      Buscar na lista
                  </span>
              )}
          </label>
          <div className="flex gap-2">
            <input 
                ...
                placeholder={type === 'WORK' ? "Nome do paciente..." : "Automático..."}
                className="w-full bg-slate-900 border border-slate-600 rounded-l p-2 text-white outline-none focus:border-indigo-500"
            />
            {type === 'WORK' && (
                <button 
                    type="button" 
                    onClick={() => setIsPatientModalOpen(true)}
                    className="bg-slate-700 hover:bg-slate-600 px-3 rounded-r border border-l-0 border-slate-600 text-white transition-colors"
      ...
      {/* MODAL DE PACIENTES (Reutilizado para Seleção) */}
      <PatientsModal 
        isOpen={isPatientModalOpen} 
        onClose={() => setIsPatientModalOpen(false)} 
        onSelect={handleSelectPatient} // Aqui acontece a mágica
      />
    </div>
  );
};
