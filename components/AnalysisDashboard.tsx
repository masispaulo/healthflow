...
const AnalysisDashboard: React.FC<AnalysisDashboardProps> = ({ analysis, procedures }) => {
  ...
  // Cálculo do P95 (Teto de Segurança - 95% dos casos caem abaixo disso)
      ...
      {fatigue && (
        <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 shadow-xl relative overflow-hidden">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center relative z-10 gap-6">
                <div>
                    <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 flex items-center gap-2">
                        <span className={`w-2 h-2 rounded-full ${fatigue.status === 'SAFE' ? 'bg-emerald-500' : fatigue.status === 'WARNING' ? 'bg-yellow-500' : 'bg-red-500'}`}></span>
                    ...
                    <div className="flex gap-4 mt-4 text-xs font-medium text-slate-400 bg-slate-900/50 p-2 rounded-lg inline-flex border border-slate-700/50">
                        <div title="Trabalho Dia">☀️ {Math.floor(fatigue.dayMinutes/60)}h <span className="opacity-50">Dia</span></div>
                        <div className="w-[1px] bg-slate-700"></div>
                        <div title="Trabalho Noite (1.8x)">🌙 {Math.floor(fatigue.nightMinutes/60)}h <span className="opacity-50">Noite</span></div>
                        <div className="w-[1px] bg-slate-700"></div>
                        <div title="Sono (-1.75/h)" className="text-emerald-400">💤 {Math.floor(fatigue.recoveryMinutes/60)}h <span className="opacity-50">Recup.</span></div>
                    </div>
                </div>

                <div className={`px-6 py-4 rounded-xl border flex flex-col items-end min-w-[180px] ${fatigue.status === 'CRITICAL' ? 'bg-red-500/10 border-red-500/30' : fatigue.status === 'WARNING' ? 'bg-yellow-500/10 border-yellow-500/30' : 'bg-emerald-500/10 border-emerald-500/30'}`}>
            ...
            <div className="w-full bg-slate-900/50 h-3 rounded-full mt-8 overflow-hidden relative">
                <div className="absolute top-0 bottom-0 w-[2px] bg-slate-600 z-20" style={{ left: `${(16.9/25)*100}%` }} title="Alerta (16.9)"></div>
                <div className="absolute top-0 bottom-0 w-[2px] bg-red-900 z-20" style={{ left: `${(20/25)*100}%` }} title="Crítico (20.0)"></div>
                <div 
                    className={`h-full transition-all duration-1000 ease-out ${fatigue.status === 'CRITICAL' ? 'bg-gradient-to-r from-red-600 to-red-500' : fatigue.status === 'WARNING' ? 'bg-gradient-to-r from-yellow-500 to-yellow-400' : 'bg-gradient-to-r from-emerald-600 to-emerald-400'}`} 
      ...
      {analysis && analysis.mean > 0 ? (
        <div className="bg-slate-800 rounded-xl border border-slate-700 shadow-xl overflow-hidden">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 divide-y lg:divide-y-0 lg:divide-x divide-slate-700">
                ...
                <div className="lg:col-span-2 p-6 flex flex-col items-center justify-center bg-slate-800/50">
                    ...
                    {/* BARRA DE ESTATÍSTICAS (O que estava faltando!) */}
                    <div className="grid grid-cols-3 gap-4 w-full mt-6 pt-6 border-t border-slate-700/50">
                        <div className="text-center group hover:bg-slate-700/30 p-2 rounded transition-colors">
                            ...
                            <p className="text-2xl font-black text-white">{mean.toFixed(1)} <span className="text-sm font-normal text-slate-500">min</span></p>
                        </div>
                        <div className="text-center group hover:bg-slate-700/30 p-2 rounded transition-colors border-l border-r border-slate-700/50">
                            ...
                            <p className="text-2xl font-black text-indigo-400">{p95.toFixed(1)} <span className="text-sm font-normal text-indigo-400/70">min</span></p>
                        ...
                        <div className="text-center group hover:bg-slate-700/30 p-2 rounded transition-colors">
                            ...
                            <p className="text-2xl font-black text-slate-400">±{stdDev.toFixed(1)} <span className="text-sm font-normal text-slate-600">min</span></p>
                        </div>
                    </div>
                </div>
                ...
                <div className="lg:col-span-1 p-8 bg-slate-900/30 flex flex-col justify-center relative">
                    ...
                    <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-6 text-center">Simulador de Risco</h3>
                    ...
                    <div className="mb-8">
                        ...
                        <p className="text-[10px] text-slate-500 mt-2 text-center text-balance">
                    ...
                    <div className="text-center bg-slate-800/50 rounded-xl p-6 border border-slate-700/50">
                        ...
                        <p className={`text-5xl font-black tracking-tighter transition-colors duration-300 ${riskProbability > 50 ? 'text-red-500' : riskProbability > 20 ? 'text-yellow-400' : 'text-emerald-400'}`}>
        ...
        <div className="p-12 bg-slate-800/30 rounded-xl border border-dashed border-slate-700 text-center flex flex-col items-center justify-center gap-3">
            ...
            <p className="text-sm text-slate-500 max-w-sm">
                Cadastre pelo menos <strong className="text-indigo-400">2 procedimentos de Trabalho</strong> (não conta descanso) para que a IA trace sua curva de consistência.
            </p>
        </div>
      )}
    </div>
